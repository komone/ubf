%% This is the original UBF(A) encoder/decoder by 
%% Joe Armstrong. All I have done is to update it
...
decode_init() ->
	{ more, fun(I) -> ubf:decode(I, [[]], dict:new()) end }.

decode(Str) -> 
    %% io:format("decode:~s~n",[Str]),
    decode(Str, decode_init()). %get_init()).
